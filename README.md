...
다만 이에 대한 단점으로 Base64로 인코딩된 이미지는 위와 같이 일반적으로 매우 긴 문자열로 표현됩니다. 이러한 긴 문자열은 마크다운 문서의 소스 코드를 직접 보거나 편집할 때 가독성이 매우 저하됩니다. 특히, 이미지가 많거나 인코딩된 문자열이 긴 경우 문서의 전체적인 관리가 어려워집니다.
